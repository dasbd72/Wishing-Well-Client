...
export function listTasks(roomId, userId, selectActive) {
  return (dispatch, getState) => {
      ...
      return listTasksFromApi(roomId, userId, selectActive).then(posts => {
...
export function AorRTask(taskId, isAccepted) {
  return (dispatch, getState) => {
      dispatch(startLoading());
      return AorRTaskFromApi(taskId, isAccepted).then(posts => {
          dispatch(listTasks());
      }).catch(err => {
          console.error('Error creating tasks', err);
      }).then(() => {
...
export function childrenComplete(taskId) {
  return (dispatch, getState) => {
      dispatch(startLoading());
      return childrenCompleteFromApi(taskId).then(posts => {
          dispatch(listTasks());
      }).catch(err => {
          console.error('Error creating tasks', err);
      }).then(() => {
...
export function parentVerify(taskId) {
  return (dispatch, getState) => {
      dispatch(startLoading());
      return parentVerifyFromApi(taskId).then(posts => {
          dispatch(listTasks());
      }).catch(err => {
          console.error('Error creating tasks', err);
      }).then(() => {
